  ...
  <div class="hello">
    ...
    <div class="container">
      <b-button
        ...
        style="float: right"
        >{{ address ? truncate1(address, 15) : "Connect Metamask" }}</b-button
      >
      <b-card class="custom-card" v-if="didDoc">
        ...
        <!-- <b-card class="mt-4">
          <div class="text-left">
            ...
            <div class="form-group row">
              <label class="col-sm-2 col-form-label"
                ><strong>Controller:</strong></label
              >
              <div class="col-sm-10 d-flex align-items-center">
                <p class="mb-0">
                  {{ edvClient.verificationMethod.controller }}
                </p>
              </div>
            </div>
          </div>
          <div class="text-right">
            <b-button variant="primary" @click="fetchAllVcFn"
              >Fetch Your Documents</b-button
            >
          </div>
        </b-card>
        <b-card v-if="fetchEncryptedCred.length" class="mt-4">
          <h4 class="text-left">
            <strong> Your Encrypted Credetials </strong>
          </h4>
          <div class="row mt-4 ml-4">
            <div v-for="(cred, index) in fetchEncryptedCred" :key="index">
              ...
              >
                ...
                <b-button variant="primary" @click="decryptVc(cred)"
                  >Decrypt VC</b-button
                >
              </b-card>
            </div>
        ...
        <b-tabs>
          ...
          <b-tab title="Schema" :active="activeTab === 'tab2'">
            <p v-if="showSchemaRegStatus" class="mt-4">
              Issue Verifiable Credential for recently registered Schema<a
                class="ml-2"
                @click="goToTab('tab3')"
                >{{ signedSchema.id }}</a
              >
          ...
          <b-tab title="Verifiable Credential" :active="activeTab === 'tab3'">
            <b-tabs class="mt-4">
              <b-tab title="Issue Credential">
                <p v-if="vcId" class="mt-4 blink">
                  Resolve Verifiable Credential Status and Perform Credential
                  Revocation<a
                    class="ml-2"
                    style="cursor: pointer"
                    @click="goToTab('tab4')"
                ...
                <div class="form-group row pt-4">
                  <label class="col-sm-2 col-form-label" for="schemaName"
                    ><strong
                      >Schema ID<span style="color: red">*</span>:</strong
                    ></label
                  >
                  <div class="col-sm-10">
                    <input
                ...
                <br />
                <p class="mt-4" v-if="issueCredAttributes.length">
                  Enter values for Credential Attributes
                </p>
                <div
                  v-for="(fields, index) in issueCredAttributes"
                  :key="index"
                >
                  <input
                    type="text"
                ...
                <div class="mr-4 mt-4">
                  <b-button @click="issueCred" v-if="issueCredAttributes.length"
                    >Sign and Issue VC</b-button
                  >
                  <b-button
                    class="ml-2"
                    v-if="issuedCred !== null"
                    @click="syncEdv"
                    >Store Credential in EDV</b-button
                  >
                </div>
              </b-tab>
              <b-tab title="Resolve Credential" :active="activeTab === 'tab4'">
                <div class="form-group row pt-4">
                  <label class="col-sm-2 col-form-label" for="schemaName"
                    ><strong
                      >VC ID<span style="color: red">*</span>:</strong
                    ></label
                  >
                  <div class="col-sm-10">
                    <input
                ...
                <div class="pt-2" v-if="credStatus">
                  <b-button @click="updateCredentialStatus('SUSPENDED')"
                    >Suspend VC</b-button
                  >
                  <b-button
                    class="ml-2 mr-2"
                    @click="updateCredentialStatus('Live')"
                    >Live VC</b-button
                  >
                  <b-button @click="updateCredentialStatus('REVOKED')"
                    >Revoke VC</b-button
                  >
                </div>
              </b-tab>
              <b-tab title="Fetch Credential" :active="activeTab === 'tab5'">
                <div class="text-right mt-4">
                  <b-button @click="fetchAllVcFn"
                    >Fetch All VC from EDV</b-button
                  >
                </div>
                <div class="row mt-4 ml-4">
                  <div v-for="(cred, index) in fetchEncryptedCred" :key="index">
    ...
    <hf-pop-up Header="Schema Register" Id="reg-schema" Size="xl">
      <div class="text-center">
        <p v-if="showSchemaRegStatus">
          View Schema<a
            class="ml-2"
            :href="`https://explorer.hypersign.id/hypersign-testnet/schemas/${signedSchema.id}`"
            target="_blank"
            >{{ signedSchema.id }}</a
          >
        </p>
    ...
    <hf-pop-up
      Header="Decrypted Verifiable Credential"
      Id="decrypted-vc"
      Size="xl"
    >
      <json-viewer
  ...
  methods: {
    ...
    async resolveVcStatus() {
      try {
        ...
        const verificationResult =
          await this.hypersignVC.resolveCredentialStatus({
    ...
    async updateCredentialStatus(statusToUpdate) {
      try {
        ...
        const updatedCredResult = await this.hypersignVC.updateCredentialStatus(
          params
        );
    ...
    async syncEdv() {
      const dataToAddEdv ={
        content:this.issuedCred
      }
      console.log(dataToAddEdv)
    ...
    async issueCred() {
      try {
        if (this.didDoc === null) {
          throw new Error(
            "Connect your Metamask and generate DID Document in DID Tab"
          );
        }
        if (this.schemaId === "") {
